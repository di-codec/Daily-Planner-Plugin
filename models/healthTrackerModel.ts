...
//         const offset = (day + 6) % 7; // Сдвиг так, чтобы неделя начиналась с понедельника
...
//         if (file) {
//             new Notice(`Файл "${file.name}" уже существует!`);
//             return;
//         }
...
//         const muscleGroups = ["Gluties", "Legs", "Back", "Brists", "Sholders", "Jogging", "Joga"];

//         let content = `# Health Tracker - ${new Date().toLocaleDateString("en-GB")}\n\n`;
//         content += "| Weekdays           | " + daysOfWeek.map(day => `${day}`).join(" | ") + " |\n";
//         content += "| ------------------ " + "|".repeat(7) + "|\n";
//         content += "| Daily Habits Track | " + dates.map(d => `${d}`).join(" | ") + " |\n";

//         for (const group of muscleGroups) {
//             content += `| ${group.padEnd(18)} | ` + dates.map(() => `<input type="checkbox" unchecked> `).join(" | ") + " |\n";
//         }

//         file = await this.app.vault.create(filePath, content);
//         new Notice(`Файл "${file.name}" создан!`);
//     }
