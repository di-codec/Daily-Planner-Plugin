...
export class SummaryManager {
  ...
  /**
   * Главная функция: собирает статистику и создаёт Summary.md
   */
  public async generateWeeklySummary(): Promise<void> {
    const stats = await this.collectStats();
    const summaryContent = this.generateSummaryMarkdown(stats);

    const summaryFilePath = "Summary.md";
    let summaryFile = this.app.vault.getAbstractFileByPath(summaryFilePath) as TFile;
  ...
  /**
   * Сбор статистики из Health Tracker и Daily Notes
   * ⚡️ Сейчас данные-заглушки. Подключи сюда реальный парсинг.
   */
  private async collectStats(): Promise<SummaryStats> {
    // Примерные данные, потом заменить парсингом файлов
    const workoutsThisWeek = 5;
    const focusMuscle = "Legs";
    ...
    // Бар-чарт (Mon–Sun)
    const barData = [4, 3, 1, 2, 2, 0, 0];

    // Круговая диаграмма
    const pieData = [
      { label: "Legs", value: 8 },
      { label: "Back", value: 6 },
      { label: "Glutes", value: 5 },
      { label: "Shoulders", value: 4 },
    ];

    // Сравнение с прошлой неделей
    const deltaWorkouts = 2;
    const deltaTasks = 10;
  ...
  /**
   * Генерация Markdown для Summary.md
   */
  private generateSummaryMarkdown(stats: SummaryStats): string {
    const { workoutsThisWeek, focusMuscle, focusPercent, tasksCompleted, tasksTotal, streak,
      barData, pieData, deltaWorkouts, deltaTasks } = stats;
...
## 🔑 Key Metrics
- 🏋️ Workouts this week: **${workoutsThisWeek}**
- 🔥 Focused on: **${focusMuscle} (${focusPercent}%)**
- ✅ Tasks completed: **${tasksPercent}%**
...
xychart-beta
    ...
    y-axis "Exercises" 0 --> 6
    bar ${JSON.stringify(barData)}
\`\`\`

### Focus by Muscle Group
\`\`\`mermaid
pie showData
${pieData.map((p) => `    "${p.label}" : ${p.value}`).join("\n")}
\`\`\`

---

## ✅ Task Summary
- Tasks completed: **${tasksCompleted} / ${tasksTotal} (${tasksPercent}%)**
- Average per day: **${(tasksCompleted / 7).toFixed(1)} tasks**
...
- ${deltaWorkouts >= 0 ? "+" : ""}${deltaWorkouts} workouts  
- ${deltaTasks >= 0 ? "+" : ""}${deltaTasks}% tasks completed
`;
  }
}
