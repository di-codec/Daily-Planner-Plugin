...
📊 **Average per day:**  ${(PrevWeekTotalFinishedTasks / 7).toFixed(1)} tasks`;
    ...
    private async readWeekFile(date: Date): Promise<{ [muscleGroup: string]: number[] }> {
        ...
        muscleGroups.forEach(group => {
            habitData[group] = new Array(9).fill(0);
        });
...
//                     PrevAllUnfinishedTasks.push(...PrevUnfinishedTasks);
//                     PrevWeekAllContent += PrevContent + "\n";
//                 }
...
//     /**
//      * Read Health Tracker data from a specific week file, extracting muscle groups dynamically from the current week
//      */
//     private async readWeekFile(date: Date): Promise<{ [muscleGroup: string]: number[] }> {
//         const monthName = this.getFileNameByMonth(date);
//         const weekName = this.getFileNameByWeek(date);
...
//             const habitData: { [muscleGroup: string]: number[] } = {};
//             DEFAULT_MUSCLE_GROUPS.forEach(group => {
...
//         try {
//             const content = await this.app.vault.read(file);
//             // First, extract all muscle groups from the current file's table
//             const muscleGroups = this.extractMuscleGroupsFromContent(content);
//             console.log(`Muscle groups from current week: ${muscleGroups}`);
//             console.log(`Length: ${muscleGroups.length}`);
...
//             // Now parse the data using the extracted groups
//             const weekHabitData = this.parseMarkdownTable(content, muscleGroups);
...
//     /**
//      * Extract muscle groups dynamically from the markdown content of the current file
//      */
//     private extractMuscleGroupsFromContent(content: string): string[] {
//         const lines = content.split("\n");
//         const muscleGroups: string[] = [];
//         let inTable = false;

//         for (const line of lines) {
//             if (line.includes("Daily Habits Track")) {
//                 inTable = true;
//                 continue;
//             }
...
//                 if (cells.length < 2 || cells[0].includes("Weekdays") || cells[0].includes("---")) continue;
...
//     /**
//      * Parsing a markdown table with dynamic muscle groups (no filtering, process all found)
//      */
//     private parseMarkdownTable(content: string, muscleGroups: string[]): { [muscleGroup: string]: number[] } {
//         const lines = content.split("\n");
//         const habitData: { [muscleGroup: string]: number[] } = {};
...
//                 const muscleGroup = columns[1].trim();
//                 // Removed the include check to process all groups found
